    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                Console.WriteLine("\n=== Testing Cancellation Validation ===");
                var cancelledAuditFile = CreateCancelledAuditFile();
                cancelledAuditFile.SaveToOutputDirectory("demo_cancelled_saft.xml");
                
                Console.WriteLine("✓ Cancelled SAF-T file generated: demo_cancelled_saft.xml");
                
                // Validate the cancelled file
                var cancelledXml = XmlUtils.SerializeToXml(cancelledAuditFile);
                var cancelledErrors = SchemaValidator.Validate(cancelledXml);
                
                if (cancelledErrors.Count == 0)
                {
                    Console.WriteLine("✓ Cancellation validation passed - all rules satisfied");
                }
                else
                {
                    Console.WriteLine("✗ Cancellation validation found issues:");
                    foreach (var error in cancelledErrors)
                    {
        ...
        static AuditFile CreateCancelledAuditFile()
        {
            var auditFile = new AuditFile
            {
                Header = new Header
                    ...
                    SalesInvoices = new SalesInvoices
                    {
                        ...
                        Invoices = new List<Invoice>
                        {
                            new Invoice
                            {
                                ...
                                DocumentStatus = new DocumentStatus
                                {
                                    InvoiceStatus = InvoiceStatus.A,
                                    InvoiceStatusDate = DateTime.Now.AddDays(1).ToString("yyyy-MM-dd"),
